	...
	SpecialEnd:	
		...
		iret		; InteraptReturn - Возвращаемся с учётом и сегмента и смещения, вытащив перед этим флаги из стека (они кладутся туда при каждом прерывании)  

	StdEnd:	
		pop es ds di si bx ax

		db 0eah 	; Вызывает jump far который требует после себя адрсов
old_ofs         dw 0		; а вот и адреса
old_seg		dw 0            ; их мы кладём в начале проги
	...
	;	    so code of "Toxic" function can be executed after programms end
	;
	; Damages:  ax, dx
	;==================================================================
	GetCode proc
	...
	;	    about what to do after interrupt in bx
	;
	; Returns:  instead of standart function, "Toxic" will be called to process interrupt
	;
	; Damages:  ax
	;==================================================================
	ChangeIntFunc proc
